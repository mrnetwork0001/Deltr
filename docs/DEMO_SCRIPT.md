...
- [ ] `npx @modelcontextprotocol/inspector --transport http --server-url http://127.0.0.1:8000/mcp` → `tools/list` shows 18 tools; close it.
...
| Time | Beat | Do / say |
|---|---|---|
| 0:00–0:15 | **Hook** | Dashboard already live. Point at: mode `PAPER`, three venue dots with source labels (`bsc-mainnet-chain`, `binance-futures-testnet`, `binance-spot-mirror`), equity $10 000, drawdown 0 %, "gate median (measured on this machine; 1.5 to 2.5 µs across our runs)". *"Deltr runs one trade: long BNB on PancakeSwap V3, short the same quantity of the Binance perp. Delta-neutral by construction. Fifteen agents in this hackathon built a policy layer; this one built the strategy underneath it."* |
